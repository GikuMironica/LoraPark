...
class ParkingSensorController extends ChangeNotifier {
   Logger _logger = GetIt.I.get<LoggingService>().getLogger((ParkingSensorController).toString());
   ...
   ParkingSensorController({ParkingStateRepository stateRepo, ParkingAverageRepository avgRepo, ParkingEventRepository eventRepo}){
   ...
   Future<void> getParkingAverageDuration(int days) async{
     ...
     var startDate = endDate.subtract(Duration(days: days));
     _logger.d("Fetching data");
     _avgData = await _averageRepository.getByTime(ids:[Sensors.parking_one, Sensors.parking_two, Sensors.parking_three, Sensors.parking_four],start: startDate, end: endDate);
     _logger.d(_avgData);
     notifyListeners();
   }
   ...
   Future<void> getParkingEvents(int days) async {
     ...
     var startDate = endDate.subtract(Duration(days: days));
     _logger.d("Fetching data");
     _eventData = await _eventRepo.getByTime(ids: [Sensors.parking_one, Sensors.parking_two, Sensors.parking_three, Sensors.parking_four], start: startDate, end: endDate);
